...
function toCamelCase(name, uppercase) {
  //Splitting the variable by the - character and putting it into new array
  let variableName = name.split('-');

  let i = 1;

  //For loop that runs through the entire variable via its length
  for (i = 0; i < variableName.length; i++) {
    //Checking if the index is 0 and if uppercase is false, then make
    //the first index lowercase
    if (i === 0 && !uppercase) {
      variableName[i] = variableName[i].toLowerCase();
    } else {
      //This is to deal with the stupid multiple dash error
      //Code checks the length to see if it's BIGGER THAN 0
      //Personal reference:
      //slice() is copying method
      if (variableName[i].length > 0) {
        variableName[i] = variableName[i][0].toUpperCase() + variableName[i].slice(1);
      }
    }
  }

  //Joining elements back together
  let result = variableName.join('');

  //Optional convert of CamelCase

  //Potential code if previous to uppercase fails
  //if(uppercase){

  //Result is go to the character at the first element, make it upper case
...
function createLinkTag(rel, href) {
  ...
  //Using regular expession + test to test for special character
  if (/[\s\t\n\r]/.test(rel)) {
    rel = '"' + rel + '"';
  }

  //Using regular expression + test to test for special character
  if (/[\s\t\n\r]/.test(href)) {
    href = '"' + href + '"';
  }
...
function parseDateString(value) {
  //If the type isn't an actual string (integer/null) it will not be a string
  if (typeof value !== 'string') {
    //Throws error "value isn't a string"
    throw new Error(
      'invalid date string, expected a `YYYY/MM/DD` or `DD/MM/YYYY` formatted string'
    );
  }

  //If it's too many digits or too few including the slash, throw an error
  if (value.length !== 10) {
    throw new Error('value does not match YYYY/MM/DD or DD/MM/YYYY format');
  }

  //Split the slash to expect 3 elements
  let date = value.split('/');

  //If array is not 3 elements long, it's not a split because it passes dashes
  if (date.length !== 3) {
    throw new Error('value does not contain slashes');
  }

  //Determine YYYY/MM/DD vs DD/MM/YYYY
  let leadingYear = date[0].length === 4;
  let year = leadingYear ? date[0] : date[2];

  //Subtracting by one because of the month starting at 0
  let month = parseInt(date[1]) - 1;
  let day = leadingYear ? date[2] : date[0];
...
function toDateString(value, format) {
  try {
    ...
    if (format === 'YYYY/MM/DD') {
      //Getting the values of all the year/month/day
      let year = value.getFullYear();

      //Adding a 0 via padStart method, converting to string to use padStart
    ...
    throw new Error('an invalid date causes an error to be thrown');
  }
}
...
function normalizeTime(value) {
  //Splitting via :
  value = value.split(':');
  //Create new element to check for AM/PM by separating the space
  ...
  //Storing time into readable variables
  let hours = parseInt(value[0]);
  let minutes = parseInt(value[1]);
  let seconds = parseInt(value[2]);

  //If our new array is PM, add the 12 hours for 24 hour clock
  if (ampm) {
    if (ampm[1] === 'PM' && hours <= 12) {
      hours = hours + 12;

      //If our new array is NOT pm, do nothing
    } else if (ampm[1] === 'AM' && hours === 12) {
      hours = 0;
    }

    //If the time is PM but is not 24 hours and gives 13
  ...
  //If the time is invalid in general
  if (hours >= 24 || minutes >= 60 || seconds >= 60) {
    return null;
  }

  //If the time si 24 hours and the minutes is too much
  if (hours === 24 && minutes > 0) {
    return null;
  }
  //After parsing, turn it back into a string to get rid of square brackets
...
function formatTimes(...values) {
  //Using .filter to go look through each element in the array (like a for loop)
  //Personal reference:
  //filter() is a copying method
  //filter sometimes needs to use "function"and enclose code with ()
  ...
  //Personal reference: map is copying method
...
function mimeTypeFromFilename(filename) {
  ...
  //Making sure switch cases check the last element
  switch (lastElement) {
    case 'txt':
      return 'text/plain';
    case 'html':
...
function buildApiEndpoint(resourceType, resourceId, extraData) {
  //It's not passing and I don't know how to make it and this was
  // the only code that worked
  if (
    resourceType === null ||
  ...
  //if extraData is not there, only show the following:
